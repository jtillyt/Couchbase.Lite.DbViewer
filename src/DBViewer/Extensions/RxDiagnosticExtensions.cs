...
namespace DbViewer.Extensions
{
    public static class RxDiagnosticExtensions
    {
        ...
        {
            return source.Do(_=> Debug.WriteLine($"{logName}: OnNext - ThreadId - {Environment.CurrentManagedThreadId}"),
                            exception=> Debug.WriteLine($"{logName}: OnException - ThreadId - {Environment.CurrentManagedThreadId}. Exception: {exception} "),
