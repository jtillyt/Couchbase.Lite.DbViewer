...
namespace DbViewer.ViewModels
{
	public class DatabaseBrowserViewModel : NavigationViewModelBase, INavigatedAware
	{
			...
			: base(navigationService)
		{
			...
			AddDocumentCommand = ReactiveCommand.CreateFromTask(ExecuteAddDocumentAsync);
		...
		private async Task ExecuteAddDocumentAsync(CancellationToken cancellationToken)
		{
			var result = await _dialogService.ShowDialogAsync(DialogNames.NewDocument);

			if (result.Parameters.Any())
			{
				var documentKey = result.Parameters.GetValue<string>(DialogNames.DocumentKeyParam);
				var documentBody = result.Parameters.GetValue<string>(DialogNames.DocumentBodyParam);

				var documentInfo = new DocumentInfo(_currentDatabaseItemViewModel.Database.RemoteDatabaseInfo, documentKey,"1.0", documentBody);

				var document = await _hubService.SaveDocument(documentInfo, cancellationToken);

				if (document!=null)
				{
					await ExecuteDatabaseRefreshAsync(cancellationToken);
				}
			}
		}
